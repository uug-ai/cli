...
// RunDLQ runs provider-neutral dead-letter inspection, replay, and test seeding.
func RunDLQ(args []string, stdout, stderr io.Writer) int {
	if len(args) < 1 || (args[0] != "inspect" && args[0] != "replay" && args[0] != "seed") {
...
func parseDLQFlags(action string, args []string, stderr io.Writer) (dlqCommandConfig, error) {
	...
	flags.DurationVar(&config.timeout, "timeout", time.Minute, "administrative operation timeout after connection setup")
